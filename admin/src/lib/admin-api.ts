...
let rawBase = (import.meta.env.VITE_ADMIN_API_URL as string | undefined) ?? "http://localhost:8000/api/admin";
rawBase = rawBase.replace(/\/+$/, ""); // remove trailing slashes

if (!rawBase.endsWith("/api/admin")) {
...
export const ADMIN_API_BASE: string = rawBase;

/** Auth endpoints live at /api/auth, not /api/admin. */
export const AUTH_API_BASE: string = ADMIN_API_BASE.replace(/\/api\/admin$/, "/api/auth");
